...
- `seo:` front matter is all-or-nothing: include all seven keys (`page_description`, `canonical_url`, `featured_image`, `featured_image_alt`, `author_twitter_handle`, `open_graph_type`, `no_index`). Nulls are fine; `no_index` must be a real boolean.
...
- trailingSlash is "always": every internal link ends in `/` (e.g., `/about/`). This is enforced in `astro.config.mjs`.
